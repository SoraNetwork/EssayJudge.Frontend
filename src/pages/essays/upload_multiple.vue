<template>
  <div>
    <h1 style="font-size: 24px; font-weight: 500; margin-bottom: 16px;">批量上传作文并批改</h1>
...
const beforeUpload: UploadProps['beforeUpload'] = (file) => {
  const currentFiles = selectedFiles.value ? [...selectedFiles.value] : [];
...
function onDropFile(e: DragEvent) {
  ...
  if (e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files.length > 0) {
    ...
    if (combinedFiles.length > maxFiles) {
        ...
        fileList.value = combinedFiles.slice(0, maxFiles).map(f => ({
        ...
        fileList.value = combinedFiles.map(f => ({
          uid: Math.random().toString(),
          name: f.name,
          status: 'done',
          url: URL.createObjectURL(f)
